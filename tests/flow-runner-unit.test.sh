...
assert(validateFlowRunPolicy({ environment: "local", riskClass: "read", startUrl: "http://169.254.169.254/latest/meta-data/", steps: [{ kind: "find", by: "text", value: "Open", action: "hover" }] }).ok === false, "flow policy: metadata startUrl refused");
assert(validateFlowRunPolicy({ environment: "local", riskClass: "read", startUrl: "http://127.0.0.1:43210/fixture", steps: [{ kind: "find", by: "text", value: "Open", action: "hover" }] }, { phase: "run" }).ok === true, "flow policy: localhost fixture allowed in local run");
...
assert(validateFlowRunPolicy({ name: "danger", environment: "live-action", riskClass: "effectful", irreversibleAt: 0, steps: [{ kind: "find", by: "text", value: "Confirm", action: "click" }] }, { phase: "run", runMode: "live-action", allowlist: "danger", liveActionApprove: "danger" }).reason === "live-action flow requires AQA_LIVE_DRY_RUN_PASSED=1 or the flow name", "flow policy: live-action requires dry-run evidence before approval");
assert(validateFlowRunPolicy({ name: "danger", environment: "live-action", riskClass: "effectful", irreversibleAt: 0, steps: [{ kind: "find", by: "text", value: "Confirm", action: "click" }] }, { phase: "run", runMode: "live-action", allowlist: "danger", liveDryRunPassed: "danger", liveActionApprove: "danger" }).ok === true, "flow policy: live-action requires mode + allowlist + dry-run + approval");
