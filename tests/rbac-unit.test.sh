...
assert.deepEqual(rbac.ROLES, ['viewer', 'operator', 'owner', 'admin'], 'role order is stable');
...
for (const permission of rbac.PERMISSIONS) {
	...
	assert.equal(rbac.can('admin', permission), true, `admin can ${permission}`);
}

assert.equal(rbac.can('auditor', 'read'), false, 'unknown role fails closed');
assert.equal(rbac.can('owner', 'delete'), false, 'unknown permission fails closed');
assert.match(rbac.authorize('auditor', 'read').reason, /unknown role/, 'unknown role gets explicit denial reason');
...
assert.deepEqual(rbac.permissionsForRole('admin'), rbac.PERMISSIONS, 'admin permissions are listed');
assert.deepEqual(rbac.permissionsForRole('auditor'), [], 'unknown role has no permissions');
...
assert.equal(badEnvActor.role, 'admin', 'admin env role is preserved for audit');
assert.equal(rbac.can(badEnvActor, 'read'), true, 'admin env role can read');
