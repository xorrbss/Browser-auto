...
function saveWebuiJob(db, rec, updatedAt = Date.now()) {
	...
	const row = {
		...
		cancel_requested_at: _toMs(rec.cancelRequestedAt || rec.cancel_requested_at),
		timed_out: _toIntBool(rec.timedOut || rec.timed_out),
		enqueued_at: _toMs(rec.enqueuedAt || rec.enqueued_at) || updatedAt,
		claimed_at: _toMs(rec.claimedAt || rec.claimed_at),
		started_at: _toMs(rec.startedAt || rec.started_at),
		ended_at: _toMs(rec.endedAt || rec.ended_at),
		...
		last_heartbeat_at: _toMs(rec.lastHeartbeatAt || rec.last_heartbeat_at),
		claim_expires_at: _toMs(rec.claimExpiresAt || rec.claim_expires_at),
		...
		delete_after: _toMs(rec.deleteAfter || rec.delete_after),
