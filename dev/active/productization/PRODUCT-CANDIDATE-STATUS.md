...
| `guest_samsungdisplay_argos_readonly_smoke` | Playwright, operator-only; development integration PASS | Samsung Display guest system read-only smoke opens the authenticated application screen directly and asserts the request page is loaded. Local development integration replay passed with `RUN_ID=dev-20260612-212451-89` using exact allowlist `https://guest.samsungdisplay.com`; no owner approval or evidence pack is required unless promoted to production-open. |
