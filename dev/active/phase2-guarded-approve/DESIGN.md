# Phase 2 — Guarded Approval Execution (design v3)

**Status:** DESIGN ONLY (no code). v3 supersedes v2 after `REDTEAM-v2.md` returned **REVISE-FIRST**
on one gate-blocking HIGH (`PRESENCE-1`). v3 chooses the stronger product/security direction:
...
| T12 | TOCTOU verify→click + native confirm leg unmatchable | Acknowledge the full fingerprint re-verify is host-side, not in-batch. The batch immediately before click carries only a last-moment identity/visibility guard, and the binding authority is the consent fingerprint plus post-click I6 re-open. Disable/avoid auto-advance, capture URL before/after the click and abort on uninitiated URL delta, split confirm schema into `none|dom|native`, and fail closed unless §12 proves every confirm leg. |
...
[결재 view]  미결 목록(결정론 쿼리)에서 1건 [승인 검토] 클릭
   ...
   ▼ [approve 잡]  bin/approve-doc.sh --app A --doc D --consent-file f --comment-file c
     ...
     6. 성공검증(I6): server-fresh 재오픈/cache-bust 또는 미결함 이탈 cross-check →
        doc_id로 재오픈 → idLabel 재확인 → 이 문서가 자기 결재선에 완료 마커
        '전엔 없음 / 후엔 있음' 전이 확인. 로그인 리다이렉트/예상외 URL ⇒ ABORT(uncertain)
     7. detail_after 동기 캡처; terminal audit + status를 **하나의 트랜잭션**:
        confirmed→approved, failed/interrupted→approve_failed. 재조정은 non-terminal crash gap만 처리.
   │
...
"approve": {
  ...
  "body":    { "fromHeadingLevel": 2, "untilMarker": "결재선" },    // hash exactly what the human sees
...
- `amount` is mandatory for any high-value flow. If the app lacks a deterministic amount field, the
  approve modal must make raw body primary, collapse/de-emphasize AI summary, and either fail closed for
  high value or require a §12-pinned deterministic operator-entered amount check.
...
- `approvals.status` lifecycle: `fetched → approving (claim, same txn as nonce consume + OOB proof
  ...
  `failed`/`interrupted`). A doc in `approving` with only a non-terminal audit is re-resolved by the
  reconciliation pass (re-open by doc_id, positively verify, then confirmed/failed) before it can ever
  be re-offered. A doc in `approving` with a terminal audit is swept to `approve_failed`. `status` is
  already excluded from the list-resync COALESCE.
...
- **Consent token / signer boundary (I3 structural):** the signer receives a verified trusted ceremony result and
...
the 승인 affordance disappear after approval? Additionally for v3: can the Windows native helper display
the doc binding (doc_id/title/amount-or-no-amount/fingerprint/body digest/action) outside same-origin
page JS and require explicit per-item approve + OS credential/Windows Hello? if WebAuthn is considered,
