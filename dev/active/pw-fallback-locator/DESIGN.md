...
- [ ] **Stage 2 (레코더 자동 폴백) — 진행 예정**
  - `bin/capture.js` `candidatesFor`: 시맨틱이 count≠1로 다 실패할 때 text-xpath→css 폴백 후보 push (+ `countCandidate` 확장). `primary` 자동승격 금지 → needs_review 유지(웹UI가 선택지로 노출)
  - `bin/build-flow.js` / `webui/flows.js`: 새 by-종류가 records→flow / resolveStep 경로를 통과하는지 점검 (현재 화이트리스트는 flow-runner에만 있었음)
  - 캡처 유닛테스트
